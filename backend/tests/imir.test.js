...
let admin;
const getAdmin = async () => (admin ??= (await adminAgent()).agent);

/** Queues a simulated QA32 lot and runs the SAP pull. Returns the IMIR created for it. */
async function inwardLot({ itemCode, vendorCode = 'V100', qty = 500, plant = '1115', description = 'Test bracket' }) {
...
const REL = { section: 'RELIABILITY', checkpoint: 'Static load', specification: '200 kg for 5 min', frequencyMonths: 6 };

/** Creates the item (if needed) and approves a format for it through the API. */
async function approveFormat(itemCode, checkpoints = [DIM, VIS]) {
...
const inspector = async (plant = '1115') => agentWithRoles([{ roleCode: 'IQC_INSPECTOR', plantId: await plantId(plant) }]);
const cellsFor = (imir, section, values) => {
