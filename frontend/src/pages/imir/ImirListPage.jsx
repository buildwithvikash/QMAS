...
export default function ImirListPage() {
  ...
  const chips = [
    ...
    ...ruleChips(list, LIST_FIELDS.imirs),
  ].filter(Boolean);
    ...
    <div>
      ...
      <div className="p-5">
        <DataTable columns={columns} rows={data?.rows} loading={isFetching} error={error} sort={list.sort} onSort={list.toggleSort} tableId="imirs"
          leading={<Tabs tabs={GROUPS} active={list.filters.statusGroup ?? 'ALL'} onChange={(k) => list.setFilter('statusGroup', k === 'ALL' ? undefined : k)} />}
          filter={{ fields: LIST_FIELDS.imirs, value: list.filters.filter, onChange: (v) => list.setFilter('filter', v), storageKey: 'imirs' }}
