...
export default function DeviationListPage() {
  ...
  const chips = [
    ...
    ...ruleChips(list, LIST_FIELDS.deviations),
  ].filter(Boolean);
    ...
    <div>
      ...
      <div className="p-5">
        <DataTable columns={columns} rows={data?.rows} loading={isFetching} error={error} sort={list.sort} onSort={list.toggleSort} tableId="deviations"
          leading={<Tabs tabs={TABS} active={filters.tab ?? 'ALL'} onChange={(k) => list.setFilter('tab', k)} />}
          filter={{ fields: LIST_FIELDS.deviations, value: filters.filter, onChange: (v) => list.setFilter('filter', v), storageKey: 'deviations' }}
