...
export default function FormatLibraryPage() {
  ...
  return (
    <div>
      ...
      <div className="p-5">
        <DataTable tableId="formats"
          leading={<Tabs tabs={FILTERS.map(([value, label]) => ({ key: value ?? 'ALL', label }))} active={list.filters.status ?? 'ALL'} onChange={(k) => list.setFilter('status', k === 'ALL' ? undefined : k)} />} rowKey="itemId" columns={columns} rows={data?.rows} loading={isFetching} error={error} sort={list.sort} onSort={list.toggleSort}
